...
import { TagInputWithParent } from './TagInputWithParent';
import { useTagGraph } from '../hooks/useTagGraph';
import type { DagTag } from '../types/dag';
...
function NodeProperties() {
  ...
  const activePaletteId =
    activeFile?.paletteId ||
    (activeFile?.themeColors && activeFile.themeColors.length > 0 ? 'xmind' : 'vibrant');

  // FR: Hook pour gérer le DAG de tags
  // EN: Hook to manage tag DAG
  const { addTag } = useTagGraph();
        ...
        <div className="panel-content" style={{ paddingRight: '8px' }}>
          ...
          >
            ...
            <span style={{ fontSize: 12 }}>
              Ouvrez un fichier pour voir les paramètres de la carte
            </span>
          </div>
        </div>
      </div>
          ...
          {!mapSettingsCollapsed && (
            <div style={{ display: 'grid', gap: 12 }}>
                  ...
                  <div style={{ display: 'grid', gap: 12 }}>
                      ...
                      <div ref={paletteRef} style={{ position: 'relative' }}>
                        ...
                        >
                          <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            <span>
                              {(activePaletteId === 'xmind'
                                ? xmindPalette
                                : COLOR_PALETTES.find(p => p.id === activePaletteId)
                              )?.name || 'Sélectionner'}
                            </span>
                              ...
                              return sortedPalettes.map(p => (
                                <button
                                  ...
                                  style={{
                                    ...
                                    background:
                                      activePaletteId === p.id ? 'rgba(0,0,0,0.02)' : 'transparent',
                                  }}
                                  title={p.name}
                                >
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={e =>
                          updateActiveFileMapStyle({ backgroundColor: e.target.value })
                        }
                        style={{
                          width: '100%',
                          height: 36,
                      ...
                      >
                        {[
                          ...
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              updateActiveFileMapStyle({ backgroundPattern: id as any })
                            }
                            style={{
                              padding: '8px 12px',
                              ...
                              borderRadius: 6,
                              background:
                                activeFile?.mapStyle?.backgroundPattern === id
                                  ? 'rgba(0,0,0,0.02)'
                                  : '#fff',
                              fontSize: 12,
                              display: 'flex',
                        ...
                        activeFile?.mapStyle?.backgroundPattern !== 'none' && (
                          <div>
                            ...
                            <input
                              ...
                              onChange={e =>
                                updateActiveFileMapStyle({
                                  backgroundPatternOpacity: parseFloat(e.target.value),
                                })
                              }
                              style={{
                                width: '100%',
                            ...
                            >
                              {Math.round(
                                (activeFile?.mapStyle?.backgroundPatternOpacity || 0.3) * 100
                              )}
                              %
                            </div>
                          </div>
                      ...
                      {[
                        ...
                        <button
                          ...
                          style={{
                            ...
                            borderRadius: 6,
                            background:
                              activeFile?.mapStyle?.linkStyle === id ? 'rgba(0,0,0,0.02)' : '#fff',
                            fontSize: 12,
                            display: 'flex',
                            alignItems: 'center',
                            gap: 6,
              ...
              >
                ...
                {!styleCollapsed && (
                  <div style={{ fontSize: 12, color: '#64748b' }}>Styles à implémenter</div>
                )}
              </div>
            </div>
          )}
                      ...
                      <div className="form-group">
                        <label htmlFor="np-tags">
                          <Tag
                            ...
                            style={{ width: 14, height: 14, display: 'inline', marginRight: 4 }}
                          />
                          Tags
                        </label>
                        ...
                        {/* FR: Input pour ajouter un nouveau tag avec parent optionnel */}
                        {/* EN: Input to add new tag with optional parent */}
                        <TagInputWithParent
                          placeholder="Ajouter un tag..."
                          showParentSelector
                          onAddTag={(tagLabel, parentId) => {
                            if (!activeFile?.content || !selectedNodeId) return;

                            console.log(
                              '🏷️ NodeProperties: Ajout du tag - nœud:',
                              selectedNodeId,
                              'tag:',
                              tagLabel,
                              'parent:',
                            ...
                            // FR: 1. Créer le tag dans le DAG avec le parent si spécifié
                            // EN: 1. Create tag in DAG with parent if specified
                            const newDagTag: DagTag = {
                              id: tagLabel.toLowerCase().replace(/\s+/g, '-'),
                              label: tagLabel,
                            ...
                            // FR: 2. Ajouter le tag au nœud
                            // EN: 2. Add tag to node
                            const command = new AddTagCommand(selectedNodeId, tagLabel);
                            const newMap = command.execute(activeFile.content);
                            console.log('🏷️ NodeProperties: Carte mise à jour avec nouveau tag');

                            // FR: 3. Émettre l'événement pour la synchronisation
                            // EN: 3. Emit event for synchronization
                            console.log(
                              '🔥 NodeProperties: Émission de node:tagged - nœud:',
                              selectedNodeId,
                              'tag:',
                              tagLabel
                            );
                            setTimeout(() => {
                              eventBus.emit(
                                'node:tagged',
                                { nodeId: selectedNodeId, tagId: tagLabel },
                                'mindmap'
                              );
                            }, 0);

                            // FR: 4. Mettre à jour l'état
                            // EN: 4. Update state
                            useOpenFiles.setState(state => ({
                              openFiles: state.openFiles.map(f =>
                                f.id === activeFile.id ? { ...f, content: newMap } : f
