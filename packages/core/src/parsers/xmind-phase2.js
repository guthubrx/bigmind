...
import { AssetUtils, AssetPosition } from '../assets';
/**
...
export class XMindPhase2Parser {
     ...
     */
    static async extractImagesFromZip(zip) {
        const images = {};
        // FR: Chercher les images dans le répertoire 'images' ou 'media'
        // EN: Look for images in 'images' or 'media' directories
        const imageFiles = Object.keys(zip.files).filter((name) => (name.startsWith('images/') || name.startsWith('media/')) &&
     ...
     */
    static extractNodeAssets(topicData, imageLibrary) {
        const assets = [];
        ...
        if (topicData.href && typeof topicData.href === 'string') {
            ...
            const hrefImage = Object.values(imageLibrary).find((img) => img.fileName === topicData.href ||
        ...
        if (topicData.markers && Array.isArray(topicData.markers)) {
            topicData.markers.forEach((marker) => {
                if (marker.markerId) {
     ...
     */
    static enrichMindMapWithAssets(mindMap, images) {
        return {
            ...mindMap,
            assetLibrary: {
                images,
     ...
     */
    static enrichMindMapWithTheme(mindMap, themeId) {
        return {
            ...mindMap,
            themeId,
        };
     ...
     */
    static async createXMindFromMindMap(mindMap) {
        const zip = new JSZip();
     ...
     */
    static convertMindNodeToXMindTopic(mindMap) {
     ...
     */
    static nodeToXMindTopic(node, mindMap) {
        const topic = {
            id: node.id,
            title: node.title,
        };
     ...
     */
    static async exportMindMapAsXMind(mindMap, fileName) {
        try {
     ...
     */
    static validatePhase2Compatibility(mindMap) {
        const warnings = [];
        // FR: Vérifier les images
        // EN: Check images
        if (mindMap.assetLibrary?.images) {
            const totalSize = Object.values(mindMap.assetLibrary.images).reduce((sum, img) => sum + img.size, 0);
            if (totalSize > AssetUtils.DEFAULT_SIZE_LIMIT) {
                warnings.push(`Total des images (${AssetUtils.formatFileSize(totalSize)}) dépasse la limite`);
            }
        }
...
//# sourceMappingURL=xmind-phase2.js.map