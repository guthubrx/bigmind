...
export class XMindParser {
     ...
     */
    static extractThemeColors(themeData) {
     ...
     */
    static convertSheetJSONToBigMind(sheetData) {
        const rootTopic = sheetData.rootTopic || sheetData.root || sheetData.topic;
        if (!rootTopic) {
     ...
     */
    static parseJSONTopic(topicData) {
        ...
        if (topicData.children) {
            if (topicData.children.attached && Array.isArray(topicData.children.attached)) {
                ...
                topicData.children.attached.forEach((child) => {
    ...
    static parseXML(xmlText) {
        ...
        if (!workbook) {
            ...
            throw new Error(`Fichier .xmind invalide : structure XML non reconnue. ` +
     ...
     */
    static parseTopic(topicElement) {
        ...
        const id = topicElement.getAttribute('id') ||
     ...
     */
    static async parseSimple(arrayBuffer) {
        try {
            const zip = await JSZip.loadAsync(arrayBuffer);
     ...
     */
    static convertToBigMind(xMindMap) {
        const nodes = {};
     ...
     */
    static parseFromJSON(jsonContent) {
        try {
    ...
    static parseNodeRecursive(xmindNode, mindMap, parentId, x, y) {
        ...
        // FR: Créer le nœud
        // EN: Create the node
        const node = NodeFactory.createNode(xmindNode.title || xmindNode.text || 'Nœud sans titre', parentId, xmindNode.style);
    ...
    static serializeNode(node, mindMap) {
        ...
        if (node.children && node.children.length > 0) {
            xmindNode.children = node.children.map((childId) => {
